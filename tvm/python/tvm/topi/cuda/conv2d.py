...
@autotvm.register_topi_compute("conv2d_bw_filter_cudnn.cuda")
def conv2d_bw_filter_cudnn(
    cfg, data, kernel, strides, padding, dilation, groups=1, layout="NCHW", out_dtype="float32"
):
    """Compute conv2d using CuDNN library"""
    if layout == "NCHW":
...
@autotvm.register_topi_schedule("conv2d_bw_filter_cudnn.cuda")
def schedule_conv2d_bw_filter_cudnn(cfg, outs):
    """Create the schedule for conv2d_cudnn"""
    return generic.schedule_extern(outs)
