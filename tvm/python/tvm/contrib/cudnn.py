...
def conv_backward_filter(x, w, pad, stride, dilation, conv_mode, tensor_format, algo, conv_dtype, groups=1):
    """
    ...
    OH = (x.shape[2] + pad[0] * 2 - w.shape[2]) // stride[0] + 1
    OW = (x.shape[3] + pad[1] * 2 - w.shape[3]) // stride[1] + 1

    # oshape[0] = w.shape[1]
    # oshape[1] = x.shape[1]
    oshape = list((w.shape[1], x.shape[1], OH, OW))
    # print(oshape)
    # This picks CUDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_PRECOMP_GEMM
    # It seems this is the fastest among algorithms that are always applicable
    algo = 1
    ...
    return te.extern(
        ...
        [x, w],
        lambda ins, outs: tvm.tir.call_packed(
            "tvm.contrib.cudnn.conv2d.backward.filter",
            conv_mode,
            tensor_format,
            algo,
            pad[0],
