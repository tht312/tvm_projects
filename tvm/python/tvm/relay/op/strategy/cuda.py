...
@conv2d_bw_filter_strategy.register(["cuda", "gpu"])
def conv2d_bw_filter_strategy_cuda(attrs, inputs, out_type, target):
    """conv2d cuda strategy"""
    strategy = _op.OpStrategy()
    data, kernel = inputs
    stride_h, stride_w = attrs.get_int_tuple("strides")
    ...
    assert layout == "NCHW", "only support nchw for now"
    assert padding[0] == padding[2] and padding[1] == padding[3],  "only support symmetric padding"
    
    if target.kind.name == "cuda":
        strategy.add_implementation(
            wrap_compute_conv2d(
                topi.cuda.conv2d_bw_filter_cudnn, need_data_layout=True, has_groups=True
            ),
            wrap_topi_schedule(topi.cuda.schedule_conv2d_bw_filter_cudnn),
            name="conv2d_bw_filter_cudnn.cuda",
            plevel=25,
        )
    else:
