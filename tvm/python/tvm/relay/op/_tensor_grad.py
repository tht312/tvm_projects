...
def conv2d_grad(orig, grad):
    ...
    pad_list = [(0, 0), (0, 0), (fpad_top, fpad_bottom), (fpad_left, fpad_right)]
    data = _nn.pad(data, pad_list, pad_value=0)
    backward_weight = _nn.conv2d_bw_filter(
        data,
        grad,
        strides=attrs.strides,
        padding=[0,0],
    ...
    backward_weight = reverse(backward_weight, axis=-1)
    backward_weight = reverse(backward_weight, axis=-2)
...
@register_gradient("nn.conv2d_transpose")
def conv2d_transpose_grad(orig, grad):
    """Gradient of conv2d_transpose"""
    attrs = orig.attrs
    data, weight = orig.args
    data_shape = get_const_tuple(data.checked_type.shape)
    weight_shape = get_const_tuple(weight.checked_type.shape)
    _, _, grad_h, grad_w = get_const_tuple(orig.checked_type.shape)
    batch, in_channel, in_h, in_w = data_shape
    _, out_channel, filter_h, filter_w = weight_shape
    ...
    stride_h, stride_w = get_const_tuple(attrs.strides)
    dilation_h, dilation_w = get_const_tuple(attrs.dilation)
    outpad_h, outpad_w = get_const_tuple(attrs.output_padding)
    out_h = (in_h - 1) * stride_h - fpad_top - fpad_bottom + dilation_h * (filter_h - 1) + outpad_h + 1
    out_w = (in_w - 1) * stride_w - fpad_left - fpad_right + dilation_w * (filter_w - 1) + outpad_w + 1

    assert attrs.data_layout == "NCHW", "only support NCHW data layout"
    assert attrs.kernel_layout == "OIHW", "only support OIHW kernel layout"
    assert attrs.out_layout in ["", "NCHW"], "only support NCHW output layout"

    backward_data = _nn.conv2d(
        grad,
        weight,
        channels=in_channel,
    ...
    grad = tile(grad, [in_channel // attrs.groups, 1, 1, 1])
    grad = reshape(grad, [-1, 1, 0, 0])  # batch * oc * ic // groups, 1, oh, ow
    data = _nn.dilate(data, [1, 1, stride_h, stride_w])
    data = reshape(data, [1, -1, 0, 0])  # 1, batch * ic, ih, iw

    new_padding = [filter_h-fpad_top-1, filter_w-fpad_left-1, filter_h-fpad_bottom-1, filter_w-fpad_right-1]
    ...
    padded_weight_grad_h = (
        grad_h - (in_h - 1) * stride_h - 1 + fpad_top + fpad_bottom - outpad_h
    ) // dilation_h + 1
    padded_weight_grad_w = (
        grad_w - (in_w - 1) * stride_w - 1 + fpad_left + fpad_right - outpad_w
    ) // dilation_w + 1
    ...
    if padded_weight_grad_h > filter_h or padded_weight_grad_w > filter_w:
        backward_weight = strided_slice(
            backward_weight,
            begin=[0, 0, 0, 0],
            end=[in_channel // attrs.groups, out_channel, filter_h, filter_w],
        )

    return [backward_data, backward_weight]    
    
@register_gradient("device_copy")
def device_copy_grad(orig, grad):
    """
    Returns grad on device_copy
    """
    attrs = orig.attrs
    src = attrs['src_dev_type']
    dst = attrs['dst_dev_type']
    ...
    out = device_copy(grad,dst,src)
    
    return [out]
    
@register_gradient("nn.dilate")
def dilate_grad(orig, grad):
    """
    Returns grad on nn.dilate
    """
    attrs = orig.attrs
    ...
    data = orig.args[0]
    data_shape = get_const_tuple(data.checked_type.shape)
    in_c, out_c, h, w = data_shape
    
    i_w = arange(const(w), dtype="int32")
    i_w = multiply(i_w, const(s_w))
    i_w = expand_dims(i_w, axis=0)
    i_w = expand_dims(i_w, axis=0)
    i_w = expand_dims(i_w, axis=0)
    i_w = repeat(i_w, repeats=in_c, axis=0)
    i_w = repeat(i_w, repeats=out_c, axis=1)
    i_w = repeat(i_w, repeats=s_h*(h-1)+1, axis=2)
    grad = gather(grad, 3, i_w)
    
    i_h = arange(const(h), dtype="int32")
    i_h = multiply(i_h, const(s_h))
    i_h = expand_dims(i_h, axis=1)
    i_h = expand_dims(i_h, axis=0)
    i_h = expand_dims(i_h, axis=0)
    i_h = repeat(i_h, repeats=in_c, axis=0)
    i_h = repeat(i_h, repeats=out_c, axis=1)
    i_h = repeat(i_h, repeats=w, axis=3)
    grad = gather(grad, 2, i_h)
    
    return [grad]
    
@register_gradient("reverse")
def reverse_grad(orig, grad):
    """
    Returns grad on nn.dilate
    """
    attrs = orig.attrs
