...
// relay.nn.conv2d_bw_filter
TVM_REGISTER_GLOBAL("relay.op.nn._make.conv2d_bw_filter")
    .set_body_typed([](Expr data, Expr weight, Array<IndexExpr> strides, Array<IndexExpr> padding,
                       Array<IndexExpr> dilation, int groups, IndexExpr channels,
                       Array<IndexExpr> kernel_size, String data_layout, String kernel_layout,
                       String out_layout, DataType out_dtype) {
...
RELAY_REGISTER_OP("nn.conv2d_bw_filter")
    .describe(R"code(2D convolution layer (e.g. spatial convolution over images).

This layer computes the grad for filter of 2d convolution.
...
- **out**:  This depends on the `layout` parameter. Output is 4D array of shape
    ...
    .set_num_inputs(2)
    .add_argument("data", "Tensor", "The input tensor.")
    .add_argument("weight", "Tensor", "The original output tensor.")
    .set_support_level(2)
    .add_type_rel("Conv2dBwFilter", Conv2DFIlterRel<Conv2DAttrs>)
    .set_attr<FInferCorrectLayout>("FInferCorrectLayout", ConvInferCorrectLayout<Conv2DAttrs>);
