...
template <typename AttrType>
bool Conv2DFIlterRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
               const TypeReporter& reporter) {
  ICHECK_EQ(types.size(), 3);
  const auto* data = types[0].as<TensorTypeNode>();
  const auto* weight = types[1].as<TensorTypeNode>();
  ...
  const auto trans_kernel_layout = tir::BijectiveLayout(kernel_layout, kNCHW);
  if (!trans_kernel_layout.defined()) {
    reporter->GetDiagCtx().Emit(
        Diagnostic::Error(reporter->GetSpan())
        << "conv2d only support kernel layouts that are convertible from NCHW."
        << " The provided layout is: " << kernel_layout);
    return false;
  }
  ...
  if (param->kernel_size.defined() && param->channels.defined()) {
    ...
    wshape = {{dshape_nchw[0], param->channels, param->kernel_size[0], param->kernel_size[1]}};
  ...
  // dilation
  Array<IndexExpr> oshape({channels, dshape_nchw[1], 0, 0});
