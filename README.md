...
5.  使用时直接运行python_projects下的文件即可，各个文件的具体功能如下：   
    ...
    + graph_plan_memory是内存分配优化，目前只做了内存分配部分，没有改调度部分，可以看到优化后的内存分配结果。文件在tvm/src/relay/backend/下，使用时需要将src/relay/ir/expr_functor.cc中348行"Visit_Expr"和349行visit_counter调换顺序。
