...
static std::unordered_map<const ExprNode*, const ExprNode*> expr_map_;
...
class StorageAllocaBaseVisitor : public transform::DeviceAwareExprVisitor {
 ...
 protected:
  ...
  // std::unordered_map<const ExprNode*, const ExprNode*> expr_map_;
 ...
 protected:
  ...
  void DeviceAwareVisitExpr_(const CallNode* call_node) final {
    ...
    if (call_node->op == CallLoweredOp()) {
      ...
      if (func_name.find("conv2d") != std::string::npos &&
          func_name.find("conv2d_tran") == std::string::npos) {
        // std::cout << "find conv2d op" << std::endl;
        auto* tuple_args = call_node->args[1].as<TupleNode>();
        auto* device_copy = tuple_args->fields[0].as<CallNode>();

        tuple_args = device_copy->args[1].as<TupleNode>();
        auto* on_device = tuple_args->fields[0].as<CallNode>();
        auto props = GetOnDeviceProps(on_device);
        auto* conv_input = props.body.as<CallNode>();
        call_lowered_props = GetCallLoweredProps(conv_input);
        func_name = call_lowered_props.lowered_func->name_hint;

        if (func_name.find("stride") != std::string::npos ||
            func_name.find("concat") != std::string::npos) {
          if (func_name.find("stride") != std::string::npos) {
            tuple_args = conv_input->args[1].as<TupleNode>();
            if (auto* concat = tuple_args->fields[0].as<CallNode>()) {
              call_lowered_props = GetCallLoweredProps(concat);
              func_name = call_lowered_props.lowered_func->name_hint;
              if (func_name.find("concat") != std::string::npos) {
                conv_input = concat;
              }
            }
          }
          ...
          if (func_name.find("concat") != std::string::npos) {
            tuple_args = conv_input->args[1].as<TupleNode>();
            auto* tuple = tuple_args->fields[0].as<TupleNode>();
            auto* tuple_input = tuple->fields[0].as<CallNode>();
            call_lowered_props = GetCallLoweredProps(tuple_input);
            func_name = call_lowered_props.lowered_func->name_hint;
            if (func_name.find("stride") != std::string::npos) {
              tuple_args = tuple_input->args[1].as<TupleNode>();
              auto* stride_input = tuple_args->fields[0].as<CallNode>();
              call_lowered_props = GetCallLoweredProps(stride_input);
              func_name = call_lowered_props.lowered_func->name_hint;
              if (func_name.find("concat") == std::string::npos) {
                expr_map_[stride_input] = tuple_input;
                // std::cout << "find conv2d!" << std::endl;
      ...
      if (func_name.find("max_pool") != std::string::npos) {
        // std::cout << "find maxpool op" << std::endl;
        auto* tuple_args = call_node->args[1].as<TupleNode>();
        auto* maxpool_input = tuple_args->fields[0].as<CallNode>();
        CallLoweredProps call_lowered_props = GetCallLoweredProps(maxpool_input);
        std::string func_name = call_lowered_props.lowered_func->name_hint;

        if (func_name.find("stride") != std::string::npos ||
            func_name.find("concat") != std::string::npos) {
          if (func_name.find("stride") != std::string::npos) {
            tuple_args = maxpool_input->args[1].as<TupleNode>();
            if (auto* concat = tuple_args->fields[0].as<CallNode>()) {
              call_lowered_props = GetCallLoweredProps(concat);
              func_name = call_lowered_props.lowered_func->name_hint;
              if (func_name.find("concat") != std::string::npos) {
                maxpool_input = concat;
              }
            }
          }
          call_lowered_props = GetCallLoweredProps(maxpool_input);
          func_name = call_lowered_props.lowered_func->name_hint;
          ...
          if (func_name.find("concat") != std::string::npos) {
            ...
            if (func_name.find("stride") != std::string::npos) {
              ...
              func_name = call_lowered_props.lowered_func->name_hint;
              expr_map_[stride_input] = tuple_input;
              // std::cout << "find maxpool!" << std::endl;
              // std::cout << func_name << std::endl;
            }
          }
        }
    ...
    if (const auto* fn = call_node->op.as<FunctionNode>()) {
      auto call = fn->body.as<CallNode>();
      if (call->op == Op::Get("nn.conv2d")) {
        // std::cout << "find conv2d func" << std::endl;
      }
    }
...
class StorageAllocator : public StorageAllocaBaseVisitor {
 ...
 protected:
  ...
  void DeviceAwareVisitExpr_(const CallNode* call_node) final {
    ...
    auto it = expr_map_.find(call_node);
    if (it != expr_map_.end()) {
      // std::cout << "find it!" << std::endl;
      // std::cout << GetRef<Expr>(it->first) << std::endl;
      this->VisitExpr(GetRef<Expr>(it->second));
    }
  }
  /*!
